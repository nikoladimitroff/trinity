...
bool Mesh::intersect(Ray ray, IntersectionData& data)
{
	...
	if (kdroot) {
        std::cout<<"KD Tree intersect"<<std::endl;
...
void Mesh::fillData(const std::vector<Vector>& vertices, const std::vector<Vector>& normals, const std::vector<Triangle>& triangles)
{
    this->uvs.clear();
    this->uvs.push_back(Vector(0,0,0));
